...
class JWOAuth_DataStore extends OAuthDataStore {/*{{{*/
	...
	function lookup_token($consumer, $token_type, $token) {/*{{{*/
		if ($token_type=='access') {
			...
			$atoken = JWMemcache::Instance('api')->Get('rt'.$token);
		}
		return $atoken;
	}/*}}}*/
	...
	function lookup_nonce($consumer, $token, $nonce, $timestamp) {/*{{{*/
		//return 0;
		return !(JWMemcache::Instance('api')->Add('on'.$consumer->key.$timestamp.$nonce, '1', 0, 3600));
	}/*}}}*/
	...
	function new_request_token($consumer) {/*{{{*/
		...
		JWMemcache::Instance('api')->Set('rt'.$key, $token, 0, 86400);
		return $token;
	}/*}}}*/
	...
	function new_access_token($token, $consumer) {/*{{{*/
		...
		try{
			...
			JWMemcache::Instance('api')->Del('rt'.$token->key);
