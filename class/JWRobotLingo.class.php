...
_STR_;
		$body = <<<_STR_
告诉我们这一刻你在做什么，我们会让你的朋友们知道！想要更多帮助，回复TIPS。
_STR_;
...
_STR_;
		...
		if ( empty($followe_user_db_row) )
			return JWRobotLogic::ReplyMsg($robotMsg, "哎呀！抱歉，你订阅的"
											. "${followe}我不认识，"
											. "请你确认输入了正确的叽歪帐号。了解更多？发送 HELP。"
										);
...
*/
		...
		$body = <<<_STR_
每当$followe_user_db_row[nameFull]更新，你都会收到消息。如果要撤销，请发送LEAVE $followe_user_db_row[nameScreen]。发送HELP了解更多。
_STR_;
...
_STR_;
		...
		$body = <<<_STR_
你已退定了$followe_user_db_row[nameFull]。在http://JiWai.de/$followe_user_db_row[nameScreen]/页面上点击订阅或发送FOLLOW $followe_user_db_row[nameScreen]可恢复订阅。
_STR_;
		$body = <<<_STR_
你已退定了$followe_user_db_row[nameFull]。
_STR_;
...
_STR_;
		...
		if ( 'nameScreen'==$invitee_type )
		{
			...
			{
				return JWRobotLogic::ReplyMsg($robotMsg, "哎呀！抱歉，你添加的"
												. "${user_input_invitee_address}我不认识，"
#. "请你输入叽歪帐号或手机号、邮件地址。了解更多？发送 HELP。"
												. "请你输入帐号或手机号、邮件地址。了解更多？发送 HELP。"
											);
			}
			...
			{
				$msg = "好消息！${invitee_address}已经是你的好友了，不用重复添加啦。";
			}
			else
			{
				...
				if ( $is_protected )
				{
					...
					{
						$msg = <<<_STR_
你向${invitee_address}发送的添加好友请求，他还没有回应，再等等看吧。
_STR_;
					}
					else if ( JWSns::CreateFriendRequest($address_user_id, $friend_user_id) )
					{
						$msg = <<<_STR_
搞定了！我们已经帮你向${invitee_address}发送了好友添加请求，希望能够很快得到${invitee_address}的回应。
_STR_;
					}
					else
					{
...
_STR_;
				...
				else	// not protected
				{
					...
					$msg = <<<_STR_
搞定了！已经将${invitee_address}添加为你的好友啦！
_STR_;
				}
			}
		...
		{
			return JWRobotLogic::ReplyMsg($robotMsg, "哎呀！抱歉，我太笨了。你添加的 $user_input_invitee_address 我不认识，请你输入手机号码或邮件地址。了解更多？发送 HELP。");
		}
		...
		if ( !empty($invitee_device_db_row) )
		{
			...
			$body = <<<_STR_
${invitee_address}在JiWai注册过！档案地址：http://JiWai.de/$invitee_user_row[nameScreen]/。我们已经帮你发送了好友添加请求。
_STR_;
			$body = <<<_STR_
${invitee_address}已经注册！我们已经帮你发送了好友添加请求。
_STR_;
		}
		else
		{
...
$address_user_row[nameFull]（$address_user_row[nameScreen]）邀请你来JiWai.de！
_INVITATION_;
			$invite_msg['email'] = <<<_INVITATION_
$address_user_row[nameFull]（$address_user_row[nameScreen]）邀请你来使用我们的服务！
_INVITATION_;

			$invite_msg['im'] = $invite_msg['email'] . <<<_INVITATION_
请回复你的英文名字或拼音，这样我们可以帮助你完成注册。
_INVITATION_;
...
_STR_;
		...
		$body = <<<_STR_
搞定了！$friend_user_row[nameFull]($friend_user_row[nameScreen])已经不再是你的好友了。
_STR_;
...
_STR_;
		...
		if ( 'web'==$send_via_device )
			return JWRobotLogic::ReplyMsg($robotMsg, "$friend_user_db_row[nameFull]已经关闭了通知，不想被挠挠。。。要不你稍后再试吧？");
...
_NUDGE_;
		$nudge_message = <<<_NUDGE_
$address_user_db_row[nameScreen]挠挠了你一下，提醒你更新！回复本消息既可更新。
_NUDGE_;

		JWNudge::NudgeUserIds(array($friend_user_db_row['idUser']), $nudge_message, 'nudge');

		$body = <<<_STR_
我们已经帮你挠挠了$friend_user_db_row[nameScreen]一下！期待很快能得到你朋友的回应。
_STR_;
		$body = <<<_STR_
我们已经帮你挠挠了$friend_user_db_row[nameScreen]一下！期待很快能得到你朋友的回应。
_STR_;
...
_STR_;
		...
		if ( ! empty($address_device_db_row) )
		{
			...
			$body = <<<_STR_
搞定了！你和$inviter_user_row[nameFull]($inviter_user_row[nameScreen])已经成为好友！回复GET $inviter_user_row[nameScreen]查看最新更新。
_STR_;
		...
		else if ( !empty($inviter_user_row) )
		{
			...
			if ( empty($invitation_id) )
				return JWRobotLogic::ReplyMsg($robotMsg, "哎呀！${inviter_name}没有邀请过你。请回复你希望使用的用户名。");


			JWSns::AcceptInvitation($invitation_id);

			return JWRobotLogic::ReplyMsg($robotMsg, "搞定了！你已经接受了${inviter_name}的邀请。请回复你希望使用的用户名。");
		}
		else
		{
			/*
...
_HELP_;
		...
		if ( empty($invitation_id) )
		{
			...
			if ( JWDevice::IsExist($address,$type) )
				return JWRobotLogic::ReplyMsg($robotMsg, "哎呀！${friend_name}并没有邀请你。发送HELP了解更多。");
			else
				return JWRobotLogic::CreateAccount($robotMsg);
		}
		...
		{
			$body = <<<_STR_
哎呀！没有找到邀请你的朋友${friend_name}，发送HELP或访问http://jiwai.de/了解更多！
_STR_;
			$body = <<<_STR_
哎呀！没有找到邀请你的朋友${friend_name}。
_STR_;
		}
		else
		{
			$body = <<<_STR_
搞定了！你没有接受$friend_db_row[nameFull]($friend_db_row[nameScreen])的邀请。发送GET $friend_db_row[nameScreen]获取$friend_db_row[nameFull]的最新更新（本短信服务免费）
_STR_;
		}
	...
	static function	Lingo_D($robotMsg)
	{
		...
		$help = <<<_STR_
D命令帮助：D 帐号 你想说的悄悄话。有问题吗？上 http://JiWai.de/ 看看吧。
_STR_;
		$help = <<<_STR_
D命令帮助：D 帐号 你想说的悄悄话。
_STR_;
		...
		if ( !JWFriend::IsFriend($friend_id, $address_user_id) )
		{
			...
			{
				return JWRobotLogic::ReplyMsg($robotMsg, "哎呀！你现在还不在${friend_name}的好友列表中，无法悄悄话他(她)。:-(");
			}
			else
			{
				return JWRobotLogic::ReplyMsg($robotMsg, "你还不在${friend_name}的好友列表中，无法悄悄话。"
									."你可以发送ADD ${friend_name}将${friend_name}添加为好友，然后期待也被对方添加为好友啦！");
			}
		}
	...
	static function	Lingo_Whoami($robotMsg)
	{
		...
		{
			$body = <<<_STR_
你是$address_user_row[nameFull] ($address_user_row[nameScreen])，叽歪档案位于：http://jiwai.de/$address_user_row[nameScreen]/ 。
_STR_;
			$body = <<<_STR_
你是$address_user_row[nameFull] ($address_user_row[nameScreen]) 。
_STR_;
		}
		else
		{
			$body = <<<_STR_
你是$address_user_row[nameScreen]，叽歪档案位于：http://jiwai.de/$address_user_row[nameScreen]/ 。设置密码请来这里：http://jiwai.de/wo/account/complete
_STR_;
			$body = <<<_STR_
