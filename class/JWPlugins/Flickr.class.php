...
class JWPlugins_Flickr
{
	...
	static public function GetPluginResult( $string )
	{
		...
		if ( $info )
		{
			...
			return array(
					'type' => 'html',
					'html' => '<div class="e_photo e_photo_flickr"><a href="' .$href. '" target="_blank" rel="nofollow"><img src="' .$src. '" title="Flickr图片" class="pic"/></a></div>',
					'types' => 'picture',
					'src' => $src,
				    );
		}
	...
	static public function GetPhotoInfoByApi( $id )
	{
		...
		if ( null==$rsp_obj || 'fail'==@$rsp_obj['stat'] || 0==@$rsp_obj['photo']['visibility']['ispublic'] )
			return array();
