<div id="set_bor">
	<div class="mar_b50 f_14">欢迎你，<b>{$g_current_user['nameScreen']}</b></div>
	<form id="f1" action="/wo/account/regok" method="post" enctype="multipart/form-data">
	<dl class="w2">
		<dt><a href="/{$g_current_user['nameUrl']}/"><img src="${JWPicture::GetUrlById($g_current_user['idPicture'], 'thumb96')}" title="{$g_current_user['nameScreen']}" width="96" height="96" /></a></dt>
		<dd>
			<div class="mar_b40"><h4>上传新头像... </h4></div>
			<div><input type="file" name="profile_image" /></div>
			<div class="f_gra">支持.jpg .gif .png格式，最大可以上传 2M 大小的图片</div>
		</dd>
		<dt></dt>
		<dd>
			<div><input type="submit" name='upload' value="&nbsp; 上传 &nbsp;" /></div>
		</dd>
	</dl>
	<div class="clear"></div>
	</form>
	<div class="dark">&gt;&gt; <a href="/wo/account/regook">跳过此步骤</a></div>
	<div class="dark">&gt;&gt; <a href="/wo/">马上开始叽歪</a></div>
</div>
<div class="clear"></div>
