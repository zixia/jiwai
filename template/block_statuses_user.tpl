...
<div class="one" id="status_{$one['id']}">
	...
	<div class="rt line">
		...
		<div class="f_gra">
			...
			<div class="rt lightbg"><a class="thread_item" rel="{$one['id']}:{$user['nameScreen']}" onclick="return JWAction.replyStatus('{$user['nameScreen']}','{$one['idUser']}','{$one['id']}');" href="/{$replyurl}/thread/{$replyid}">${$replynum ? $replynum.'条':''}回复</a><!--{if $g_current_user_id}-->&nbsp;&nbsp;&nbsp;<a href="/wo/favourites/${$is_favourited?"create":"create"}/{$one['id']}" onclick="return JWAction.toggleStar({$one['id']});" id="status_star_{$one['id']}" title="${$is_favourited?"不收藏":"收藏它"}">${$is_favourited?"不收":"收藏"}</a><!--{/if}--><!--{if $can_delete}-->&nbsp;&nbsp;&nbsp;<a href="/wo/status/destroy/{$one['id']}" class="c_note" onclick="return JWAction.doTrash({$one['id']})">删除</a><!--{/if}--></div>
		</div>
	</div>
	<div class="clear"></div>
</div>
