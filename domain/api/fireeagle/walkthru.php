...
require_once(dirname(__FILE__)."/../../../lib/FireEagle/FireEagle.php");

function main() {
	
	$fe_key = 'GebjN1oC2AgS';
	$fe_secret = 'l71m1pTJgCwZycsIb3I5GZEyvibCnYcF';
	...
	if (@$_GET['f'] == 'start') {
		...
		if (empty($loc->user->location_hierarchy)) {
			...
			foreach ($loc->user->location_hierarchy as $location) {
				...
				foreach ($l as $k => $v) {
					//if (in_array($k, array("type", "name", "latitude", "longitude", "best_guess", "geotype"))) continue; //WALKTHRU_SKIP
		...
		if (TRUE || $_SESSION['can_write']) { // fix when we get 'writable' from the 'user' response
			...
			<form method="POST">
			<p>Free-text entry: <input type="text" name="q" size="40"></p>
			<p>Place ID: <input type="text" name="place_id" size="40"></p>
			<p>Lat: <input type="text" name="lat" size="10"> Lon: <input type="text" name="lon" size="10"></p>
