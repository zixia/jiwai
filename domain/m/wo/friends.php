...
function leave($idUser, $idFriend){
    ...
    if ( JWFollower::Destroy($idFriend, $idUser) ) {
        JWSession::SetInfo( 'error', "退订成功，您将不会再在手机或聊天软件上收到$userInfo[nameScreen]的更新。");
    }else{
...
function follow($idUser, $idFriend){
    ...
    if ( JWFollower::Create($idFriend, $idUser) ) {
        JWSession::SetInfo( 'error', "订阅成功。$userInfo[nameScreen]的更新将会发送到您的手机或聊天软件上。");
    }else{
        JWSession::SetInfo( 'error', "哎呀！由于系统临时故障，您未能成为$userInfo[nameScreen]的粉丝，订阅失败了… 请稍后再试吧。");
    }
...
function nudge($idUser, $idFriend){
    ...
    if ( JWFriend::IsFriend( $idFriend, $idUser ) ){
        ...
        if( JWNudge::NudgeUserIds( array($idFriend), $nudgeMessage, 'nudge') ){
            JWSession::SetInfo('error', "我们已经帮您挠挠了$userInfo[nameScreen]一下！期待很快能得到您朋友的回应。");
        }else{
            JWSession::SetInfo('error', "哎呀！由于系统故障，挠挠好友失败了…… 请稍后再尝试吧。");
        }
    }else{
        JWSession::SetInfo('error', "您现在还不是$userInfo[nameScreen]的好友，不能挠挠。");
    }
