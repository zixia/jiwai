...
TB_HEIGHT = 0;
var TB_doneOnce = 0 ;
var TB_loadingImage = JiWai.AssetUrl( '/images/avatar/load.gif' );
...
function TB_show(caption, url, rel, options) {
	...
	if ( !$("TB_overlay") )
	{
		...
		$('TB_load').innerHTML = "<img src='" + TB_loadingImage + "' width='39' height='39'/>";
		TB_load_position();
	...
	if ( baseURL.match(imageURL) ) {
		...
		imgPreloader.onload = function() {
			...
			$("TB_window").innerHTML += "<a href='' id='TB_ImageOff' title='Close'><img id='TB_Image' src='"+url+"' width='"+imageWidth+"' height='"+imageHeight+"' alt='"+caption+"'/></a>" + "<div id='TB_caption'>"+caption+"<div id='TB_secondLine'>" + imageCount + prev.html + next.html + "</div></div><div id='TB_closeWindow'><a href='#' id='TB_closeWindowButton' title='Close'>关闭</a></div>";
		...
		if( queryStringMatch ) {
			var params = TB_parseQuery( queryStringMatch[1] );
		}
		else if( options ){
			var params = options;
		}
		...
		if(url.indexOf('TB_iframe') != -1){				
			...
			$("TB_window").innerHTML = "<div id='TB_title'><div id='TB_ajaxWindowTitle'>"+caption+"</div><div id='TB_closeAjaxWindow'><a href='#' id='TB_closeWindowButton' title='Close'>关闭</a></div></div><iframe frameborder='0' hspace='0' src='"+urlNoQuery[0]+"' id='TB_iframeContent' name='TB_iframeContent' style='width:"+(ajaxContentW + 29)+"px;height:"+(ajaxContentH + 17)+"px;' onload='TB_showWindow()'> </iframe>";
		} else {
//			$("TB_window").innerHTML = "<div id='TB_title'><div id='TB_ajaxWindowTitle'>"+caption+"</div><div id='TB_closeAjaxWindow'><a href='#' id='TB_closeWindowButton'>关闭</a></div></div><div id='TB_ajaxContent' style='width:"+ajaxContentW+"px;height:"+ajaxContentH+"px;'></div>";
...
function TB_showWindow(options){
	//$("TB_load").remove();
	//$("TB_window").setStyles({display:"block",opacity:'0'});
	
	if (TB_doneOnce==0) {
		TB_doneOnce = 1;
		var myFX = new Fx.Style('TB_window', 'opacity',{duration: 250, transition: Fx.Transitions.sineInOut, onComplete:function(){if ($('TB_load')) { $('TB_load').remove();} if(options['focus']){$(options['focus']).focus();} } }).start(0,1);
	} else {
		$('TB_window').setStyle('opacity',1);
		if ($('TB_load')) { $('TB_load').remove();}
	}
				//alert(888);$('username_or_email').focus();
}
