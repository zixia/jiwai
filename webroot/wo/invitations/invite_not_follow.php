...
$friends_rows_havefollow = $friends_rows_all[JWBuddy_Import::HAVE_FOLLOW];
$friends_rows_count = count($friends_rows);

$friends_rows_count_here = $friends_rows_count + count($friends_rows_havefollow);
...
<p class="black15bold">你共有&nbsp;<?php echo $friends_rows_count_here;?>&nbsp;个联系人在叽歪，未关注&nbsp;<?php echo $friends_rows_count;?>&nbsp;个，你可以关注他们</p>
   <div class="box2">
  ...
  <label class="pad1" for="not_follow_check">选择所有好友</label>
  </div><!-- pad -->
<?php
foreach($friends_rows  as $idFriend => $friends_row)
{
	...
	$friends_location = JWLocation::GetLocationName( $friends_info['location'] );
...
?>
  ...
  <div class="entry">
	<div class="floatleft"><input onclick="selectOne(this)" type="checkbox" name="friends_ids[]" checked="true" value="<?php echo $idFriend; ?>" /></div>
     <div class="head"><a href="<?php echo JW_SRVNAME; ?>/<?php echo $friends_info['nameUrl'];?>/" rel="contact"><img width="48" height="48" title="<?php echo $friends_info['nameFull'];?>" src="<?php echo JWPicture::GetUrlById($friends_info['idPicture']); ?>"/></a></div>
	  <div class="content"><span class="black15bold"><?php echo $friends_info['nameScreen']; ?></span>
		<div class="meta">
		<span class="floatright" title="<?php echo $friends_location?>"><?php echo mb_substr($friends_location, 0, 20,'UTF-8');?></span>< <?php echo $friends_array[0];?> >
		</div><!-- meta -->
		</div><!-- content -->
	</div><!-- entry -->
 <?php
