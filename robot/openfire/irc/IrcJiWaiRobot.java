...
public class IrcJiWaiRobot implements PacketListener, PacketFilter, MoMtProcessor {

    public static final String TALK_SERVER = "lb-02.jw";
    public static final int TALK_PORT = 5222;
    public static final String DEVICE = "irc";
    ...
    public static IrcJiWaiRobot irc_robot = null;
    ...
    static {
        ...
        mServer = config.getProperty("irc.server", System.getProperty("irc.server") );
        mAccount = config.getProperty("irc.account", System.getProperty("irc.account") );
        mPassword = config.getProperty("irc.password", System.getProperty("irc.password") );
        mTransport = config.getProperty("irc.transport", System.getProperty("irc.transport") );
        mResource = config.getProperty("irc.resource", System.getProperty("irc.resource") );
        mStatus = config.getProperty("irc.status", _mStatus );
    ...
    private void processMessage(Message m){
        ...
        Logger.log(m.getFrom() + "=>" + m.getTo()+ ":" + body);
    ...
    public static void main(String args[]) {
        irc_robot = new IrcJiWaiRobot();
        worker = new MoMtWorker(DEVICE, mQueuePath, irc_robot);
    ...
    public static void connect(){
        try {
            ...
            e.printStackTrace();
            Logger.logError("Irc Login failed");
        }
    }
    ...
    public static void realConnect() {
        try{
            ...
            irc_robot.processOfflineMessage();
            roster = con.getRoster();

            con.addPacketListener(irc_robot , irc_robot);
    ...
    public boolean mtProcessing(MoMtMessage message){
        String to = serializeXmpp(message.getAddress());
