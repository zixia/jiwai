...
public class JabberJiWaiRobot implements FileTransferListener, PacketListener, PacketFilter, MoMtProcessor {
	...
	static {
		...
		mAvatar = config.getProperty("jabber.avatar", System.getProperty("jabber.avatar") );
	...
	private static void setAvatar(XMPPConnection con) {
		try {
			final URL avatarUrl = new URL(mAvatar);
			VCard vCard = new VCard();
			vCard.setFirstName("de");
			vCard.setLastName("jiwai");
			vCard.setEmailHome("wo@jiwai.de");
			vCard.setJabberId("wo@jiwai.de");
			vCard.setOrganization("JiWai.de Inc.");
			vCard.setNickName("wo@jiwai.de");
			vCard.setField("TITLE", "Mr");
			vCard.setAddressFieldHome("STREET", "Yonghe Plaza");
			vCard.setAddressFieldWork("CTRY", "CN");
			vCard.setPhoneWork("FAX", "84195328");
			vCard.setAvatar(avatarUrl);
			vCard.save(con);
		} catch (Exception e) {
			Logger.logError("setAvatar failed");
		}
	}
