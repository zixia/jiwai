...
public class IcqJiWaiRobot implements PacketListener, PacketFilter, MoMtProcessor {

    public static final String TALK_SERVER = "lb-02.jw";
    public static final int TALK_PORT = 5222;
    public static final String DEVICE = "icq";
    ...
    public static IcqJiWaiRobot icq_robot = null;
    ...
    static {
        ...
        mServer = config.getProperty("icq.server", System.getProperty("icq.server") );
        mAccount = config.getProperty("icq.account", System.getProperty("icq.account") );
        mPassword = config.getProperty("icq.password", System.getProperty("icq.password") );
        mTransport = config.getProperty("icq.transport", System.getProperty("icq.transport") );
        mResource = config.getProperty("icq.resource", System.getProperty("icq.resource") );
        mStatus = config.getProperty("icq.status", _mStatus );
    ...
    public static void main(String args[]) {
        icq_robot = new IcqJiWaiRobot();
        worker = new MoMtWorker(DEVICE, mQueuePath, icq_robot);
    ...
    public static void connect(){
        try {
            ...
            e.printStackTrace();
            Logger.logError("Icq Login failed");
        }
    }
    ...
    public static void realConnect() {
        try{
            ...
            icq_robot.processOfflineMessage();
            roster = con.getRoster();

            con.addPacketListener(icq_robot , icq_robot);
