...
my $sms_mt_pid = open2($sms_mt_chld_out, $sms_mt_chld_in, "php $cwd/sms_mt.php") or die "open2 $!";
...
#print Dumper($chld_out);
	...
	my $linkid = '';
	$linkid = $1 if ( $head=~m#^LINKID:\s+(\d+)$#mi );
	...
	#
	print $chld_in "$mobile_no $encode_msg $server_address $linkid\n";
