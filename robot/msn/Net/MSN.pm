...
sub call {
  ...
  # XXX by zixia: skip is_buddy_online
  if ( 1 || $self->is_buddy_online($handle)) {
    $self->send('XFR', 'SB');
...
#print STDERR "ZIXIA: buddyadd($username,$fname)\n";
...
#    }
    ...
    unless ($data[1] eq 'FL'){
            #$self->buddyadd($data[5], $data[6]);
                # by zixia: why 5 & 6 ??
            $self->buddyadd($data[0], $data[1]);
        }
  } elsif ($cmd eq 'ILN') { #initial online state
    my (undef, $status, $username, $fname) = @data;
    $self->buddyupdate($username, $fname, $status);
  } elsif ($cmd eq 'NLN') { #online or state change
    my ($status, $username, $fname) = @data;
    $self->buddyupdate($username, $fname, $status);
  } elsif ($cmd eq 'FLN') { #offline
    my ($username) = @data;
